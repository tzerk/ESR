...
#' \code{folder} a zip archive can be provided. The file has to have a .zip
#' file extension and must contain all spectrum files in no subfolder.
#' 
#' @param folder \code{\link{character}} (required): directory where the
#' spectra files are stored. Alternatively, a compressed zip file can be
#' provided. See details!
#' @param file.ext \code{\link{character}}: import only files with a certain
#' extension
#' @param header \code{\link{logical}}: value indicating whether the file
#' contains the names of the variables as its first line.
#' @param sep \code{\link{character}}: the field separator character. Values on
#' each line of the file are separated by this character. If sep = '' (the
#' default for read.table) the separator is 'white space', that is one or more
...
read_ESRspectrum <- function(folder, file.ext = "txt", header = TRUE, sep = "") {
  # check if zip file
  is_zip <- function(x) {
  ...
  get_filelist <- function(x) {
    if (is_zip(x) == TRUE) {
      ...
      filelist <- list.files(folder, pattern = paste("*.", file.ext, sep = ""), 
                             ignore.case = TRUE)
      return(filelist)
    }
  }
  if (is_zip(folder) == TRUE) {
    ...
    for (i in 1:length(filelist)) {
      data <- lapply(filelist, function(x) read.table(unz(folder, x), header = header, 
                                                      sep = sep))
    }
  } else {
    filelist <- get_filelist(folder)
    # import all files into a list
    data <- lapply(filelist, function(x) read.table(paste(folder, "/", x, sep = ""), 
  ...
  ## RETURN VALUE
  ## ==========================================================================##
  newRLumDataCurve.read_ESRspectrum <- set_RLum.Data.Curve(data = t(as.matrix(data)), 
                                                           info = list(filelist), recordType = "ESR", curveType = "measured")
