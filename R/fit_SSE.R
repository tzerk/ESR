fit_SSE <-
structure(function(# Fit and plot a dose response curve for ESR data (ESR intensity against dose)
  ...
  bootstrap = FALSE,
  ### \code{\link{logical}} (with default): generate replicates of the input
  ### data for a nonparametric bootstrap. 
  bootstrap.replicates = 999,
  ### \code{\link{numeric}} (with default): amount of bootstrap replicates. 
  output.console = TRUE,
  ...
  ##--------------------------------------------------------------------------##
  ## to be a little bit more flexible the start parameters varries within a 
  ## normal distribution
  ...
  # non-linear least square fit with an SSE
  nls.fit<- function(x) {
    nls.bs.res<- try(nls(y~fit.functionEXP(a,b,c,x), #a*(1-exp(-(x+c)/b))
                         data=x,
                         start=c(a=a,b=b,c=c),
                         trace=FALSE,
                         weights=weights,
  ...
  if(output.console == TRUE) {
    ...
    if(bootstrap==TRUE) {
      cat(paste("\n\n ------------------- BOOTSTRAP RESULTS -------------------"))
      cat(paste("\n Mean (Gy)                  :", round(nls.bs.mean,2)))
      cat(paste("\n Median (Gy)                :", round(nls.bs.median,2)))
      cat(paste("\n Standard deviation (Gy)    :", round(nls.bs.sd,2)))
      cat(paste("\n ---------------------------------------------------------\n"))
    }
  ...
  else {"Dose response cuve"}
  
  xlim <- if("xlim" %in% names(extraArgs)) {c(extraArgs$xlim[1], extraArgs$xlim[2])} 
  else {c(De.solve+De.solve*0.5,(max(input.data$x)+max(input.data$x)*0.1))}
  
  ylim <- if("ylim" %in% names(extraArgs)) {c(extraArgs$ylim[1], extraArgs$ylim[2])} 
  else {c(0,max(input.data$y)+max(input.data$y)*0.1)}
  
  xlab <- if("xlab" %in% names(extraArgs)) {extraArgs$xlab} 
  else {"Dose (Gy)"}
  
  ylab <- if("ylab" %in% names(extraArgs)) {extraArgs$ylab} 
  else {"ESR intensity (a.u.)"}
  
  pch <- if("pch" %in% names(extraArgs)) {extraArgs$pch} 
  else {21}
  
  col <- if("col" %in% names(extraArgs)) {extraArgs$col} 
  else {"black"}
  ...
  if(output.plot == TRUE) {
    ...
    if(bootstrap == TRUE) {
      ...
      hist(nls.bs.res$t, breaks = "FD", freq=FALSE, 
           main = "Histogram", xlab = "Equivalent Dose (Gy)",
           col = "gray80", border = "gray66")
      
      lines(density(na.exclude(nls.bs.res$t)),
            col = "black", lwd = 1.5)
      
      # plot vertical dashed line at t0
      abline(v = c(nls.bs.res$t0, 
                   mean(na.exclude(nls.bs.res$t)),
                   median(na.exclude(nls.bs.res$t))),
             col = "black", lwd = 1.5,
             lty = c(2,3,4))
    ...
    if(class(fit)!="try-error"){
      ...
      plot(input.data[,1],input.data[,2],
           main=main,
           ylim=ylim,
           xlim=xlim,
           ylab=ylab,
      ...
      mtext(substitute(D[e] == De, 
                       list(De=paste(abs(De.solve),"+/-",De.solve.error, "Gy",
      ...
      curve(fit.functionEXP(a,b,c,x), lwd=1.5, col="black", add=TRUE, lty = 1)
  ...
  output <- try(data.frame(De=abs(De.solve),
                           ...
                           d0=d0,
                           n=length(input.data$x),
                           weigths=fit.weights), silent=TRUE)  
  ...
  ##note<<
  ## This function is largely derived from the \code{plot_GrowthCurve} function
